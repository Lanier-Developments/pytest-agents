...
describe('PrometheusMetrics', () => {
  ...
  it('increments a pre-configured counter without labels', async () => {
    metrics.incrementCounter('research_agent_documents_analyzed_total');
    const output = await metrics.getMetrics();
    expect(output).toMatch(/research_agent_documents_analyzed_total\{[^}]*\} 1/);
  });

  it('increments a pre-configured counter with labels', async () => {
    metrics.incrementCounter('research_agent_requests_total', { action: 'create' });
    const output = await metrics.getMetrics();
    expect(output).toMatch(/research_agent_requests_total\{[^}]*action="create"[^}]*\} 1/);
  });
  ...
  it('reuses an existing counter instance across calls', async () => {
    ...
    metrics.incrementCounter('research_agent_documents_analyzed_total');
    const output = await metrics.getMetrics();
    expect(output).toMatch(/research_agent_documents_analyzed_total\{[^}]*\} 2/);
  });
  ...
  it('sets a gauge value with labels', async () => {
    metrics.setGauge('research_agent_sources_added_total', 3, { type: 'article' });
    const output = await metrics.getMetrics();
    expect(output).toMatch(/research_agent_sources_added_total\{[^}]*type="article"[^}]*\} 3/);
  });
  ...
  it('observes a histogram value with labels', async () => {
    metrics.observeHistogram('research_agent_request_duration_seconds', 0.5, { action: 'create' });
    const output = await metrics.getMetrics();
    expect(output).toMatch(
      /research_agent_request_duration_seconds_sum\{[^}]*action="create"[^}]*\} 0\.5/
    );
  });
  ...
  it('starts a timer with labels', async () => {
    const stop = metrics.startTimer('research_agent_request_duration_seconds', { action: 'update' });
    stop();

    const output = await metrics.getMetrics();
    expect(output).toMatch(
      /research_agent_request_duration_seconds_count\{[^}]*action="update"[^}]*\} 1/
    );
  });
  ...
  it('includes the app default label in exported metrics', async () => {
    metrics.incrementCounter('research_agent_documents_analyzed_total');
    const output = await metrics.getMetrics();
    expect(output).toContain('app="research-agent"');
  });
